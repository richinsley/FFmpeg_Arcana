...
/* (de)muxers */
/* we need thread support for Arcana */
...
static AVOutputFormat ** arcana_muxers          = NULL;
static AVOutputFormat ** arcana_all_muxers      = NULL;
...
static AVOutputFormat ** arcana_output_devs     = NULL;
static AVOutputFormat ** arcana_all_output_devs = NULL;
...
int arcana_register_muxer(void * muxer_opaque)
{
    ...
    if(!arcana_muxers) {
        arcana_muxers = calloc(1, sizeof(AVOutputFormat *) * MAX_ARCANA_ENTRIES);
        arcana_all_muxers = calloc(1, (sizeof(AVOutputFormat *) * MAX_ARCANA_ENTRIES) + sizeof(muxer_list));
    ...
    if(muxer_opaque) {
        arcana_muxers[arcana_muxer_count] = (AVOutputFormat *)muxer_opaque;
        arcana_muxer_count++;
    }
    memcpy(arcana_all_muxers, arcana_muxers, sizeof(AVOutputFormat *) * arcana_muxer_count);
...
int arcana_register_output_device(void * dev_opaque)
{
    ...
    if(!arcana_output_devs) {
        arcana_output_devs = calloc(1, sizeof(AVOutputFormat *) * MAX_ARCANA_ENTRIES);
        arcana_all_output_devs = calloc(1, (sizeof(AVOutputFormat *) * MAX_ARCANA_ENTRIES) + (sizeof(AVOutputFormat *) * (internal_output_devs_count + 1)));
    ...
    if(dev_opaque) {
        arcana_output_devs[arcana_output_devs_count] = (AVOutputFormat *)dev_opaque;
        arcana_output_devs_count++;
    }
    memcpy(arcana_all_output_devs, arcana_output_devs, sizeof(AVOutputFormat *) * arcana_output_devs_count);
    if(internal_output_devs_count) {
        AVOutputFormat ** odev = (AVOutputFormat **)outdev_list_intptr;
        memcpy(&arcana_all_output_devs[arcana_output_devs_count], odev, sizeof(AVOutputFormat *) * internal_output_devs_count);
    }
    pthread_mutex_unlock(&arcana_outputs_mutex);
    return 0;
}
...
void avpriv_register_devices(const AVOutputFormat * const o[], const AVInputFormat * const i[])
{
    ...
    AVOutputFormat** odevs = (AVOutputFormat**)outdev_list_intptr;
    ...
    if(arcana_output_devs) {
        arcana_all_output_devs = realloc(arcana_all_output_devs, (sizeof(AVOutputFormat *) * MAX_ARCANA_ENTRIES) + (sizeof(AVOutputFormat *) * (internal_output_devs_count + 1)));
    }
    if(arcana_input_devs) {
